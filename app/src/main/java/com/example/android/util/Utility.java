        ...
        if (!TextUtils.isEmpty(response)){
            try {
                ...
                for (int i = 0; i < allProvinces.length(); i++){
                    JSONObject provincedObject = allProvinces.getJSONObject(i);
                    Province province = new Province();
                    province.setProvinceName(provincedObject.getString("name"));
                    province.setProvincedCode(provincedObject.getInt("id"));
                    province.save();
                }
                return true;
